...
async function fetchRoles() {
    ...
    const response = await fetch('/api/list/roles', {
        headers: {
...
async function fetchAdmins() {
    ...
    if (filterRoleId !== null) {
        params.append('roleId', filterRoleId);
    }

    console.log('FETCH:', params.toString());

    try {
        const response = await fetch(`/admins?${params.toString()}`, {
            headers: {
                Authorization: `Bearer ${localStorage.getItem('token')}`,
                'Content-Type': 'application/json'
            }
        ...
        console.error(err);
        showToast('Unexpected error while fetching admins', 'error');
    }
}
...
function renderRecords(records) {
    ...
    container.innerHTML = records.map(record => {
        const formattedRoleName = record.roleName
            ? record.roleName
                .replace(/_/g, ' ')
                .toLowerCase()
                .replace(/\b\w/g, c => c.toUpperCase())
        ...
        return `
            <div class="record-item">
                <div class="record-info">
                    ...
                    <div class="record-vehicleNo">${formattedRoleName}</div>
                    <div>
...
document.addEventListener('DOMContentLoaded', () => {
    fetchAdmins();
    populateRoleFilter();
    applyPermissionVisibility('addAdminBtn', 'CREATE_ADMIN');

    const filterRoleSelect = document.getElementById('filterRole');
    if (filterRoleSelect) {
        filterRoleSelect.addEventListener('change', e => {
        filterRoleId = e.target.value === 'all'? null : Number(e.target.value);
            currentPage = 0;
            fetchAdmins();
        });
    }
    ...
    modal.innerHTML = `
        ...
        <div class="modal-body">
                ...
                <div>
                    <div class="info-box">
                        <div style="display: flex; gap: 10px;">
                            ...
                            <h4>How admin creation works</h4>
                        </div>
                        <ul>
                            <li>
            ...
            <div class="detail-grid-middle">
                <div>
                    <div class="detail-item">
                        ...
                        <select class="detail-value detail-input" id="roleId">
                            ${roles.map(r => `
                                <option value="${r.roleId}">
                                    ${formatEnum(r.roleName)}
                                </option>
                            `).join('')}
                        </select>
                    </div>
            ...
            <div class="detail-grid-bottom">
                <div></div>
                <button class="btn-adv btn-save" onclick="saveNewAdmin()">Save</button>
            </div>
...
async function handleNicCheck(nicInput) {
    ...
    try {
        const response = await fetch(`/admins/check-nic/${nic}`, {
            headers: {
                Authorization: `Bearer ${localStorage.getItem('token')}`
            }
        });
        ...
        if (data.exists && data.hasAdminProfile) {
        ...
        if (data.exists && !data.hasAdminProfile && data.userId) {
            const result = await showConfirmModal({
                title: 'Details Found',
                message: 'An existing user was found with this NIC. Do you want to link to this user and create an admin profile?',
                confirmText: 'Yes',
                cancelText: 'No', 
                danger: false
            });
...
window.saveNewAdmin = async function () {
    ...
    const roleIdEl = modal.querySelector('#roleId');

    if (!fullNameEl || !emailEl || !nicEl || !roleIdEl) {
    ...
    if (!payload.fullName || !payload.email || !payload.nic || !payload.roleId) {
    ...
    try {
        ...
        if (!response.ok) {
            let message = 'Failed to create an admin';
            try {
                const err = await response.json();
                if (err.message) message = err.message;
            } catch {}
        ...
        console.error(err);
        showToast('Unexpected error while creating an admin', 'error');
    }
};

function getRoleNameById(roleId) {
    if (!cachedRoles) return '';
    const role = cachedRoles.find(r => String(r.roleId) === String(roleId));
    return role ? formatEnum(role.roleName) : '';
}
...
window.viewRecord = async function (id) {
    try {
        ...
        if (hasPermission('UPDATE_ADMIN') || hasPermission('DELETE_ADMIN')) {
            advancedSectionHtml = `
            ...
            <div class="modal-body">
                ...
                <div class="detail-grid-middle"  id="detailGrid">
                    <div>
                        <div class="detail-item">
                            <span class="detail-label">Access Role</span>
                            <span class="detail-value" id="field-roleId" data-role-id="${record.roleId}">${getRoleNameById(record.roleId)}</span>
                        </div>
                    </div>
                </div>
                ${advancedSectionHtml}
...
window.enableEdit = async function() {
    ...
    const editableFields = ['name', 'email', 'nic','vehicleNo', 'roleId'];
    ...
    try {
        ...
        console.error(e);
        showToast('Failed to load access role details', 'error');
    }
        ...
        if (field === 'roleId') {
            ...
            element.innerHTML = `
                <select style="width: 100%; padding: 8px; border: 1px solid var(--border-color); border-radius: 6px; background: var(--gray-50); color: var(--text-primary);">
                    ${roles.map(role => `
                        <option value="${role.roleId}"
                            ${String(role.roleId) === String(selectedRoleId) ? 'selected' : ''}>
                            ${formatEnum(role.roleName)}
                        </option>
                    `).join('')}
                </select>
            `;
...
window.saveRecord = async function (id) {
    try {
        const payload = {
            ...
            roleId: document.querySelector('#field-roleId select').value,
            phoneNumbers: []
        };
...
window.activateAccount = async function(id) {
    ...
    try {
        const response = await fetch(`/admins/${id}/activate`, {
            method: 'POST',
            headers: {
                'Authorization': `Bearer ${localStorage.getItem('token')}`
            }
...
window.deactivateAccount = async function(id) {
    ...
    try {
        const response = await fetch(`/admins/${id}/deactivate`, {
            method: 'POST',
            headers: {
                'Authorization': `Bearer ${localStorage.getItem('token')}`
            }
...
window.resetPassword = async function (id) {
    ...
    try {
        const response = await fetch(`/admins/${id}/reset-password`, {
            method: 'POST',
            headers: {
                Authorization: `Bearer ${localStorage.getItem('token')}`
            }
...
async function refreshAdmins() {
    ...
    const success = await fetchAdmins   ();
