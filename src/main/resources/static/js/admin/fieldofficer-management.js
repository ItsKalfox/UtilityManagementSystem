...
async function fetchFieldOfficers() {
    ...
    try {
        const response = await fetch(`/field-officers?${params.toString()}`, {
            headers: {
                Authorization: `Bearer ${localStorage.getItem('token')}`,
                'Content-Type': 'application/json'
            }
        ...
        if (!response.ok) {
            let message = 'Failed to fetch field officer';
            try {
        ...
        console.error(err);
        showToast('Unexpected error while fetching field officer', 'error');
    }
}
    ...
    container.innerHTML = records.map(record => `
        <div class="record-item">
            <div class="record-info">
                ...
                <div class="record-vehicleNo">${record.vehicleNo}</div>
                <div>
...
document.addEventListener('DOMContentLoaded', () => {
    fetchFieldOfficers();
    applyPermissionVisibility('addFieldOfficerBtn', 'CREATE_FIELD_OFFICER');
    ...
    modal.innerHTML = `
        ...
        <div class="modal-body">
                ...
                <div>
                    <div class="info-box">
                        <div style="display: flex; gap: 10px;">
                            ...
                            <h4>How field officer creation works</h4>
                        </div>
                        <ul>
                            <li>
            ...
            <div class="detail-grid-middle">
                <div>
                    <div class="detail-item">
                        ...
                        <input class="detail-value detail-input" id="vehicleNo">
                    </div>
                    <div></div>
                    <div></div>
                </div>
            ...
            <div class="detail-grid-bottom">
                <div></div>
                <button class="btn-adv btn-save" onclick="saveNewFieldOfficer()">Save</button>
            </div>
...
async function handleNicCheck(nicInput) {
    ...
    try {
        const response = await fetch(`/field-officers/check-nic/${nic}`, {
            headers: {
                Authorization: `Bearer ${localStorage.getItem('token')}`
            }
        });
        ...
        if (data.exists && data.hasFieldOfficerProfile) {
            showToast('Field Officer already exists.', 'error');
        ...
        if (data.exists && !data.hasFieldOfficerProfile && data.userId) {
            const result = await showConfirmModal({
                title: 'Details Found',
                message: 'An existing user was found with this NIC. Do you want to link to this user and create a field officer profile?',
                confirmText: 'Yes',
                cancelText: 'No', 
                danger: false
            });
...
window.saveNewFieldOfficer = async function () {
    ...
    const payload = {
        ...
        vehicleNo: vehicleNoEl.value.trim(),
        phoneNumbers: []
    };

    if (!payload.fullName || !payload.email || !payload.nic || !payload.vehicleNo) {
    ...
    if (isExistingUser) {
        payload.fieldOfficerId = linkedUserId;
    ...
    try {
        ...
        if (!response.ok) {
            let message = 'Failed to create field officer';
            try {
                const err = await response.json();
                if (err.message) message = err.message;
            } catch {}
        ...
        console.error(err);
        showToast('Unexpected error while creating field officer', 'error');
    }
};

window.viewRecord = async function (id) {
    try {
        const response = await fetch(`/field-officers/${id}`, {
            headers: {
                Authorization: `Bearer ${localStorage.getItem('token')}`,
                'Content-Type': 'application/json'
            }
        ...
        if (hasPermission('UPDATE_FIELD_OFFICER') || hasPermission('DELETE_FIELD_OFFICER')) {
            advancedSectionHtml = `
                ...
                <div class="detail-grid-middle"  id="detailGrid">
                    <div>
                        <div class="detail-item">
                            ...
                            <span class="detail-value" id="field-vehicleNo">${record.vehicleNo}</span>
                        </div>
                        <div></div>
                        <div></div>
                    </div>
...
window.enableEdit = async function() {
    ...
    const editableFields = ['name', 'email', 'nic','vehicleNo', 'areaCode'];
...
window.saveRecord = async function (id) {
    try {
        const payload = {
            ...
            vehicleNo: document.querySelector('#field-vehicleNo input').value.trim(),
...
window.deleteRecord = async function (id) {
    const result = await showConfirmModal({
        ...
        message: 'Are you sure you want to permanently delete this field officer? This action cannot be undone.',
        confirmText: 'Delete',
        cancelText: 'Cancel',
        danger: true
    });
...
window.activateAccount = async function(id) {
    ...
    try {
        const response = await fetch(`/field-officers/${id}/activate`, {
            method: 'POST',
            headers: {
                'Authorization': `Bearer ${localStorage.getItem('token')}`
            }
...
window.deactivateAccount = async function(id) {
    ...
    try {
        const response = await fetch(`/field-officers/${id}/deactivate`, {
            method: 'POST',
            headers: {
                'Authorization': `Bearer ${localStorage.getItem('token')}`
            }
...
window.resetPassword = async function (id) {
    ...
    try {
        const response = await fetch(`/field-officers/${id}/reset-password`, {
            method: 'POST',
            headers: {
                Authorization: `Bearer ${localStorage.getItem('token')}`
            }
...
async function refreshFieldOfficers() {
    ...
    const success = await fetchFieldOfficers();
